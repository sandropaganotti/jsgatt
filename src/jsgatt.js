...
class Device {
  ...
  _gatt(opts, newValue, forget = false, delay = 0) {
    ...
    return (!forget &&
      ...
      : (!forget && device && device !== LOADING
          ...
          .then(service => service.getCharacteristic(opts.characteristic))
          .then(characteristic => {
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          (newValue
            ...
            : characteristic.readValue()).then(resolve);
        }, delay);
      });
    });
  }
  ...
  _writeValue(value, characteristicKey) {
    ...
    return cache[characteristicKey].item.writeValue(value).then(() => {
  ...
  _requestDevice(deviceKey, opts) {
    ...
    return this.window.navigator.bluetooth.requestDevice(
      this._getRequest(opts)
    );
  }
